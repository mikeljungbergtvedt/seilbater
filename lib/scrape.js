...
// Domener som krever ekte browser for å komme forbi Cloudflare
const BROWSER_DOMAINS = /yachtworld\.com|boat24\.com|boattrader\.com/i;

/* ---------- Playwright på Vercel ---------- */
async function fetchViaBrowser(url) {
  // Konfigurer @sparticuz/chromium for serverless
...
function isInterstitial(html, title) {
  ...
  const patterns = [
    "one moment", "just a moment", "checking your browser",
  ...
  return sample.includes("cf-browser-verification") ||
...
export async function scrapeUrl(url) {
  ...
  try {
    ...
    // HTTP-fallback feilet — prøv med browser
    if (!useBrowser) {
  ...
  if (isInterstitial(html, acc.title)) {
    throw new Error("bot-beskyttelse (kunne ikke passere Cloudflare)");
  }
  if (acc.price == null && !acc.title) {
    throw new Error("fant hverken pris eller tittel på siden");
  }
...
export async function closeBrowser() {
  // No-op — browser lukkes per request
}
