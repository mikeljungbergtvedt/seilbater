...
// Domener som krever ScrapingBee (Cloudflare bot-beskyttelse blokkerer
// vanlig HTTP fra cloud-IPer). ScrapingBee bruker residential proxy + ekte
// browser og passerer typisk Cloudflare-utfordringen.
const SCRAPINGBEE_DOMAINS = /yachtworld\.com|boat24\.com|boattrader\.com|batagent\.se/i;

async function fetchViaScrapingBee(url) {
  const key = process.env.SCRAPINGBEE_API_KEY;
  if (!key) throw new Error("SCRAPINGBEE_API_KEY mangler i env");
  // stealth_proxy: bypass Cloudflare, DataDome osv. — 75 credits/req.
  // (premium_proxy alene er ikke sterk nok mot Yachtworld/Boat24.)
  ...
  if (!res.ok) throw new Error(`ScrapingBee HTTP ${res.status}: ${await res.text().then(t => t.slice(0, 200))}`);
...
export async function scrapeUrl(url) {
  const useBee = SCRAPINGBEE_DOMAINS.test(url);
  let res;
  try {
    res = useBee ? await fetchViaScrapingBee(url) : await fetchViaHttp(url);
  } catch (err) {
    // Fallback: prøv den andre metoden hvis første feiler
    if (useBee) throw err; // ScrapingBee er allerede fallback, gi opp
    // Vanlig HTTP feilet — prøv ScrapingBee som backup
    try { res = await fetchViaScrapingBee(url); }
    catch { throw err; }
  }
